...
import { evidenceSandbox, identity, checkpointContent } from "../lib/orchestration";
...
export default defineTool({
  ...
  async execute(_input, ctx) {
    ...
    const checkpoint = prior?.status === "complete" ? prior : await writeLaneCheckpoint(sandbox, identity, route.axis, checkpointContent(route.axis, route.axis === "engineering-quality" && route.attempt === 0), 0);
