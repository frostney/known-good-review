...
| Review orchestration | Use authored `defineWorkflowTool` and `ctx.agent` for the bounded application-owned lane/scout loop. Trusted auth fixes identity and axes; signed checkpoint attestations authorize fresh continuation. Ordinary checkpoint and report tools retain terminal validation. Preserve the 16 logical dispatch limit per invocation and native replay semantics. See [orchestration ownership and limits](architecture.md). |
...
| Eve telemetry and routing | Adopt the experimental provider layout for metadata-only model/action observations. Native identities join usage and Gateway metadata in durable session state. Retain route enrichment, cache-inclusive accounting and protocol fallback; publication, recovery, cancellation and bounded reconciliation remain in session hooks. |
| Connect / Chat / Octokit | Retain native credential resolvers and recover once from a genuine GitHub 401 through targeted token-cache eviction. Preserve explicit Chat installation scope, Eve default app context, bot identity, bounded pagination and admission deadlines. Permission, exchange and network failures remain terminal; routine admission still uses native caching. |
